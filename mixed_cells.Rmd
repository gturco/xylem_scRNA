...
ggplot(ici_tissue, aes(Tissue, group=State, fill=State)) + geom_bar(position = "dodge", colour="black") + theme(axis.text=element_text(size=10, angle = 60))  + scale_fill_manual(values=c("#000000","#FFFFFF"))

bar_chart <- ggplot(ici_tissue, aes(State, group=Tissue, fill=Tissue)) + geom_bar(position = "stack")+ theme(axis.text=element_text(size=10, angle = 60)) + scale_fill_brewer(palette = "Spectral", direction = 2) 
